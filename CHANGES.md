...
* The theme now comes from the mockups' stylesheet rather than being copied from it by hand, and a handful of things that made the program look square and grey next to the mockups are gone with the copying. Every corner was half as round as drawn, because FlatLaf and Java2D take a corner's diameter where the stylesheet gives its radius. The window between the cards was the card colour and the navigation rail the window colour, the other way round; light buttons and inputs were white where they are the second surface colour. Text was 12 pixels on Windows, because the theme never set a default font, where the mockups' body text is 13, and the semibold weight of section headings and table headers came out regular, because Java2D draws a 600 weight as 400. A greyed-out control keeps its colours and is drawn at 45 % - a disabled Start still reads as the green Start - rather than every disabled control turning the same grey. Tables have no zebra stripes. Switching between light and dark no longer leaves colours of the old theme behind, such as the job's name staying on a white chip in the dark theme: the program's own colours are looked up again every time they are painted.
* Tables read as the mockups' tables. Headers sit over their values - numbers right, everything else left - where the look and feel centred them; numbers are in monospaced regular figures in the secondary colour rather than bold Courier; a status is a small capsule in the cell rather than a colour filling it. Columns take the width their content needs: identifiers and numbers are not cut while a name can give way, and when the table is narrow the secondary columns go first and can be brought back from the column settings at the right of the tab row. A cell cut short shows its whole text on hover. Only a width you drag is remembered; the width a column happened to get on its first layout used to be stored as though it had been chosen, which is how whole tables came to be rows of ellipses. The placements table's check boxes and side badges now appear: the renderers had been swapped for the old ones before they were installed.
* The icons are the mockups' single-colour set throughout, in the theme's text colour, with the warning and error colours where the colour carried meaning. They replace 152 uses of the old coloured icons, including the ones a toolbar button used to change back to when its action was updated.
* Enumerations and class names are shown in words. Around 50 kinds of setting - backlash compensation methods, motion control types, feeder rotation modes and the rest - showed their Java constant, and 39 property sheets were titled with the class name; they now show a translated name, and a value the program keeps only for old configurations is no longer offered. Adding a feeder, a camera, an actuator or a driver lists the types by name with a sentence about each, with the class name small underneath.
* The configuration forms in the properties column fit it, until each is redone. The forms were laid out for a wide panel under the camera - fixed column widths, two label and field pairs a row, a titled border around every group - and in a 500 pixel column they scrolled sideways and cut off the bottom of their inputs. Shown in the column, a form now has its fixed widths and heights relaxed, a grid that is still too wide folded into one pair a row, and its titled borders drawn as section headings; the label beside a check box ticks it. A few forms laid out for what they show, such as the G-code console, are left as they are.
* The window's frame is drawn to the mockups. On Windows the top bar is the title bar: the window's own buttons sit at its right end, and the separate title bar, which spent 30 pixels of height on a name the top bar already shows, is gone. The camera, the page and the properties column are cards with 14 pixel corners 10 pixels apart on the window's colour, the dividers between them carry grip dots, and the 5 pixel margin round the whole window is gone. The menus are the mockups' six: the Job menu's items are in the Machine menu, the Window menu's in the View menu. While no job is under way the progress reads "Ready", a paused job's Start reads "Resume", and on a narrow window the search box shrinks to its magnifier and the job's name shortens, with the whole name on hover.
* The navigation rail is the mockups' rail: 68 pixels, the mockups' icons, the selected page on a rounded accent block with a bar at the rail's edge, rules between the groups, and settings at the foot. The feeders page's item carries a yellow count of the feeders that need attention; the issues item's count is red only when one of the issues is a warning or worse. When the window is too short for all the items with their labels, they show their icons alone rather than pushing settings off the bottom.
* Each page lays the window out for itself. The pages that work the machine - job, feeders, vision, machine setup - show the camera large with the tables under it at 320 pixels; the library pages, the issues and the log show it as a 150 pixel strip, which carries the camera choice, a compact readout and a switch between large, strip and hidden. Each page remembers what the divider was dragged to, and only a drag is remembered: a window closed in full screen used to come back with the page area at nothing. The camera is made large while a wizard's instructions are up and goes back when they are done, and the status bar shows the wizard's step with a link back to it. View › Camera on this page does the same as the strip's switch, and the multiple-window style gains View › Show camera window. The properties column stays on the pages that use it and folds away on those that never put anything in it; folded or unfolded by hand, it stays that way on that page. It is never narrower than 320 pixels or wider than 30 % of the window.
* The cards on the camera image follow the mockups. The camera selector's pills carry their icons and the scale is a glass chip of its own; the crosshair, grid, ruler and package outline switches can be on together, where one replaced the other, and the package outline of the placement selected on the job page is new. The crosshair and the outline are drawn as the mockups draw them. The light switch the camera view drew in its top right corner, as a circle of rays that looked like a busy indicator under the zoom card, is a switch among the view tools; the view's own hint, camera name and Z height moved out from under the cards; the right-click menu's reticle names are translated. The cards are 12 pixels in from the edges with a shadow under them, and the instructions go under the lower of the two rows along the top. The readout's unit sits on its value's baseline and a crowded readout changes to the 16 pixel size rather than being cut; the jog card is 328 pixels, the tool's name at 600 with the head in grey, the step reads 1 rather than 1.0, and a card too tall for the image drops its speed and its foot buttons. The folded card's keycap says Ctrl Shift J, and the feeders page starts with it folded. The instructions' step circle says "2/4" for the wizards that count their steps.
* The properties column's header is the mockups': the thing's name at 15/700 with the full name on hover, and under it what kind of thing it is and its class, "料带飞达 · ReferenceStripFeeder"; the "..." menu holds the actions the thing has, which were only on the machine setup page's toolbar. Tabs that do not fit go behind a button at the end of the row. Reset and Apply share the foot's width.
* The window fits the screen. A first start fills the screen; the window cannot be made smaller than 1024 by 640, and it opens within the part of the screen the task bar leaves, on the screen it is mostly on. A dialog taller than its screen is cut to it and scrolls.
* Help › Control gallery shows every control of the interface on one sheet, in the current theme, laid out as the mockups' design system page: the colours, the type scale, the buttons, the status capsules and chips, the inputs, the navigation and the table. It is what the new controls are checked against.
* An error says in one plain sentence what went wrong, under a title that names what failed, with the original message and its stack folded under Details and Copy details and Open log beside Close. There were 372 calls into one box titled "Error" that showed the English text as it came, and the whole stack when the message was empty. About forty of the messages met most often - the machine not enabled or not homed, a soft limit, a subject not found, the feeder, nozzle and actuator errors, time-outs, the serial port - are put in the user's words, in the display language. The same error again within five seconds is counted rather than stacked on top of the first, every error dialog opens on the window's thread, and every one is logged as an error.
* Questions name their actions on their buttons. Moving to a location before an action asks "Move there and go on / Go on without moving / Cancel", where it asked Yes, No, Cancel and No meant "don't move but do it anyway"; unapplied edits are "Apply changes / Discard changes", quitting with a job running is "Stop the job and quit", and a board that already has placements is merged or replaced by name. A delete says what and how many in its title and on its red button and lists the names, where it asked "Delete X?" in the title and again in the text with an English ", and N others" in the middle of the Chinese; the focus starts on Cancel beside a red button, so Enter does not delete. Removing placements asks now, as every other delete does. Deleting, stopping and overwriting are the only things that ask. The questions inside the older setup forms are changed as those forms are redone.
* The file dialogs have titles and show only the files they open: a job file when opening a job, a board file when adding a board, the importers' own types. On Windows they listed every file.
* The welcome window is Pono's own: the logo, three steps - enable the machine, gather the issues, calibrate - with the first one's button enabling and homing the machine, the manual and the configuration folder as links, and "Don't show again", without which it comes back at the next start. It used to title itself "!欢迎使用 Pono …!", show the upstream OpenPnP 2.0 notes read from the working directory, and close with an English OK.
* Settings is a page at the foot of the rail and replaces the appearance dialog. It holds the theme - Pono dark, Pono light or following the system; Metal, Nimbus and the other looks that bent the shell out of shape are gone - the font size, the language and the units, which moved in from the View menu, the number of decimals shown, the operator's name, autosave, the configuration folder, the keyboard shortcuts and About.
* The keyboard reaches everything a mouse does. Buttons take the focus with Tab and show a focus ring, except the ones that move the machine or run the job, which a space bar pressed for something else must not press. Ctrl+1 to Ctrl+9 go to the rail's pages. In a table Enter edits the selected cell, or the first cell along the row that can be edited, and Delete deletes the selected rows after asking; "/" puts the cursor in the page's filter from anywhere but a text field, on every page with a filter, where a table with editable cells used to start editing its cell with the slash. The command search opens with Ctrl+K from a text field too, closes with Esc whatever has the focus, and lists a command that cannot run now greyed with the reason rather than leaving it out.
* An empty table says how to get its first row, and a filtered one that nothing matches says so. A greyed button's tooltip says why it is greyed: the machine is not enabled, not homed or busy, no job is running, a running job cannot be stepped. While the machine is disconnected the state chip is red with a Connect button beside it. While the machine carries out a task outside a job - homing, a calibration, a move - the status bar shows a running bar.
...
* Group titles are readable in the dark theme. Seventeen forms drew their group borders' titles in black, and a dozen error labels in a fixed red; they take the theme's colours now. The Issues and Solutions table no longer draws its severity and state as black text on a pale fill, and the log no longer shows its DEBUG lines in black.
* Choosing Cancel when asked about unapplied edits now keeps the selection on the vision and machine setup pages, as it did on the others; those two went on to the new selection and left the edits' owner behind. The question opens over the window rather than in the middle of the primary screen, which on a second monitor was the other screen.
* The status bar says what the machine and the job are doing: "Disconnected" while the machine is off rather than "Idle", and "Paused" for a job paused by an error rather than "Running".
* Switching between the light and dark theme while nothing is selected no longer leaves the properties column's tabs in the old theme's colours - white tabs with dark text in the dark theme - once something is selected.
* Typing "/" while editing a table cell types a slash. It moved the focus to the filter box. Pressing the space bar with no placement selected no longer throws an exception on the job, board and panel pages.
* A value a table refuses is no longer dropped in silence. Eight table models - parts, packages, feeders, footprints and their pads, boards and panels, their placements and their locations - wrote the refusal to the log and put the old value back with nothing on screen; the cell now says why under itself for a few seconds, "abc" is not a number for the usual case.
* When the configuration cannot be loaded, the message no longer suggests deleting the configuration directory, which holds the whole machine's setup and its backups. It says which file failed and why, points to the backups, and offers Retry, Open the configuration folder and Quit.
* The messages shown when no browser can be opened for a link are translated.
* The diagnostics' X/Y backlash finding points at the backlash calibration Issues and Solutions offers, rather than setting the offset itself. It raised a one-sided offset to 1.2 times the backlash measured, a second way of setting what the calibration sets together with the method, and it looked at one-sided positioning only: an axis with no compensation, or a directional one of well under the backlash, was never raised however much slack it measured. Every method is now held to the backlash measured with compensation off.
* The diagnostics measure Z backlash with Z compensation switched off. They measured through it and took what was left for the offset, which then replaced the offset that had left it, so the axis ended up compensated by the residual rather than by its slack.
* A camera's fixed settle wait is proposed to cover the slower of its settling and its frame delay, with the margin; the frame delay measured on its own is the better number when the settling test did not see all of it. And once a camera's settling or latency has been measured, Issues and Solutions no longer also suggests switching it to adaptive settling, which was a second fix for the same thing pulling the other way.
...
* The values of 275 settings and the names of 93 kinds of machine element are translated into Simplified Chinese, where the drop-downs and titles showed their Java names. Forty-two short labels that read as "English (Chinese)", such as "拒绝 (Dismiss)", are Chinese only; "拼版" is "拼板" and "为锰" is "为锚" where they were misspelt, and the pad dialogs say 长 and 宽 where they said 横向长度 and 纵向宽度.
...
* A ruler for the interface: `tools/ui-ruler/ruler.ps1` starts the build on a demonstration configuration, photographs every page in dark and light at 1600 by 1000, and again at 1366 by 768 and at 125 % scaling, puts each page beside its mockup at the same pixel size, and audits what is on screen into one report. The audit looks for English or class names in the Chinese interface, table headers and labels cut down to an ellipsis, a properties column wider than itself, icons from outside the new set, font sizes off the stylesheet's scale, greyed-out controls with no word on why, controls cut off by a small window, text too close to its background to read, and the configuration wizards still to be redone. The demonstration configuration is the mockups' scene, written through the model by `UiFixture`: 48 placements beginning with the six the job mockup shows, 24 feeders under the feeder mockup's names, a 16-up panel, and a simulated top camera looking at the mockups' board. The mockups' sources moved into `design/mockups`, where `tools/ui-ruler/render-mockups.ps1` renders each in both themes. The ruler runs on preferences of its own in memory, so it does not rearrange the real program's window.
* Mockups for the rest of the redesign, in `design/mockups`: the parts, packages, boards, panels, vision, machine, log and settings pages, the issues page with its issues, measurements and machine overview tabs, the new calibration page, the error and confirmation dialogs, the welcome window, empty, busy and failed states, and two configuration forms laid out by the rules every form will follow. They share `frame.js`, which draws the shell as it will look once it is redone, with the properties column at the 500 pixels the program uses rather than the first mockups' 380.
* The design tokens have one source. `tools/design-tokens/generate.ps1` reads the colours, radii, spacing and type scale from `design/mockups/mock.css` and writes them into both themes, into `org.openpnp.gui.shell.Tokens`, and the mockups' icon set from `icons.js` into `icons/pono`; a test fails when any of them no longer matches the stylesheet.
* Configuration forms can be written as a list of fields: `Form.of(feeder).section(...).text("name", ...).length("tapeWidth", ...).location("referenceHoleLocation", ..., false).locationButtons().choice("method", ..., Method.class).visibleWhen(...).validate(...).build()`. The form lays itself out as one column that fits the properties column, binds each field through the same converters and buffered Apply and Reset as the old wizards, shows a failed check under its field, shows or hides a field by the value another has on screen, and marks the buttons that move the machine. Building it checks that every field names a real property, so a misspelt name fails the form's test rather than a user's click. The field types are text, integer, decimal, length, angle, location with the capture and move buttons, choice, segments, switch, read-only, vision pipeline, action and a block of anything else.
* The interface ruler paints each window at the scale of the screen it is laid out on. It had painted the 1600 by 1000 round at 150 % on a 125 % screen, where Chinese glyphs round to about 4 % wider than the layout measured them, and reported labels cut off that no one at either setting sees.
